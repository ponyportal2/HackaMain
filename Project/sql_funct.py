...
def sql_token_exists_in_db(token):
    ...
    try:
        result = sql(
            f"select EXISTS(select token from session where token = '{token}')").scalar()
    except:
        result = 0
    return int(result)
...
def sql_username_exists(username):
    result = sql(
        f"select EXISTS(select name from users where name = '{username}')").scalar()
    return int(result)  # ok, 10/10 very good


def sql_get_user_password_hash(username):
    result = sql(
        f"select password from users where name = '{username}'").scalar()
    return result
...
def sql_set_user_auth_status(username, status):
    sql(
        f"update users set accepted_status = {status} where name = '{username}'")
...
def sql_get_all_user_pictures_with_pattern(username, pattern):
    ...
    result = sql(
        f"select p.path from pictures p join users u on (p.user_id = u.id) where u.name = '{username}' and p.path like '{parsed}'").fetchall()
    return result
...
def sql_get_user_telegram_authkey(username):
    try:
        result = sql(
            f"select telegram_code from users where name = '{username}'").scalar()
    except:
        result = None
    return result
...
def sql_token_to_user(token):
    try:
        id = sql(
            f"select user_id from session where token = '{token}'").scalar()
...
def sql_does_image_exist(path):
    result = sql(
        f"select EXISTS(select path from pictures where path = '{path}')")
    return result


def sql_remove_image_location(user_id, filename):
    sql(
        f"delete from pictures where path = '{filename}' and user_id = '{user_id}'")


def sql_remove_image_by_pattern(user_id, pat):
    sql(
        f"delete from pictures where path like '{pat}' and user_id = '{user_id}'")
...
def sql_get_avatar(username):
    ...
    try:
        id_user = sql(
            f"select id from users where name = '{username}'").scalar()
        result = sql(
            f"select ava_path from avatar where user_id = '{id_user}'").scalar()
    except:
        result = None
    return result
...
def sql_get_user_auth_status(username):
    try:
        result = sql(
            f"select accepted_status from users where name = '{username}'").scalar()
    except:
        result = None
    return result
