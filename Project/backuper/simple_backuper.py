...
def upload_file():
    ...
    if "filename" not in request.files or "token" not in request.files or "file" not in request.files:
        return jsonify({'status': 'missing_fields'}), 410
    ...
    if token != expected_token:
        return jsonify({'status': 'invalid_token'}), 420

    file_path = filename_file.read().decode('utf-8')

    if file_path == '':
        flash('No selected file')
        return jsonify({'status': 'error'}), 430  # No selected file
    ...
    if file_content and allowed_file(file_path):
        ...
        return jsonify({'status': 'error'}), 440
